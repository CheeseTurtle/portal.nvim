...
---@field extra_opts Portal.WindowExtraOptions
...
---@class Portal.WindowExtraOptions
---@field hl_namespace? integer
---@field before_open_hook? fun(obj: Portal.Window): boolean?
---@field after_open_hook? fun(obj: Portal.Window)
---@field before_close_hook? fun(obj: Portal.Window): boolean?
---@field after_close_hook? fun(obj: Portal.Window)
...
vim.api.nvim_set_hl(0, "PortalSearch", { link = "CurSearch", default=true })
vim.api.nvim_set_hl(0, "PortalLabel", { link = "IncSearch", default = true })
...
---@param extra_opts Portal.WindowExtraOptions
---@return Portal.Window
function Window:new(label, content, options, extra_opts)
  assert(vim.api.nvim_buf_is_valid(content.buffer), ("Portal: invalid buffer %s"):format(content.buffer))

  local window = {
    label = label,
    content = content,
    options = options or vim.empty_dict(),
...
function Window:open()
  ...
  if self.extra_opts.before_open_hook and self.extra_opts.before_open_hook(self) == false then return end
  ...
  if not vim.api.nvim_buf_is_loaded(self.state.buffer) then
    ...
    if not ok then log.warn(("Window.open: unable to load buffer, reason: %s"):format(reason)) end
    if not vim.api.nvim_buf_is_loaded(self.state.buffer) then
  ...
  vim.api.nvim_set_option_value(
    ...
    table.concat({
      ...
      ("%s:%s"):format("Search", "PortalSearch"),
    }, ","),
    { win = self.state.window }
  )

  if self.extra_opts.hl_namespace then print("SET HL NAMESPACE TO: ", self.extra_opts.hl_namespace); vim.api.nvim_win_set_hl_ns(self.state.window, self.extra_opts.hl_namespace) end
  ...
  if self.extra_opts.after_open_hook then
    local tf, msg = pcall(self.extra_opts.after_open_hook, self)
    if not (tf or vim.in_fast_event()) then vim.api.nvim_err_writeln(msg) end
  end
end

function Window:add_label()
  if not self.state then log.error("Window.label: window is not open.") end
...
function Window:has_label(label) return self.label == label end
...
function Window:close()
  ...
  if self.extra_opts.before_close_hook and self.extra_opts.before_close_hook(self) == false then return end
  if vim.api.nvim_win_is_valid(self.state.window) then vim.api.nvim_win_close(self.state.window, true) end
  ...
  self.state = nil
  if self.extra_opts.after_close_hook then
    local tf, msg = pcall(self.extra_opts.after_close_hook, self)
    if not (tf or vim.in_fast_event()) then vim.api.nvim_err_writeln(msg) end
  end
end
